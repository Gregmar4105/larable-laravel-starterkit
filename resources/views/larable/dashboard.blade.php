...
@section('styles')
<link rel="stylesheet" href="{{ asset('css/larable.css') }}">
@endsection
...
async function loadTableData(tableName) {
    ...
    try {
        const resp = await fetch(`/larable/database/table/${tableName}`);
...
function switchDbView(view) {
    ...
    if (view === 'schema') {
        document.getElementById('db-structure-view').style.display = 'none';
        document.getElementById('db-schema-view').style.display = 'block';
        hideAllViews();
        document.getElementById('db-view').style.display = 'block';
        loadERDiagram();
    } else {
